...
const LoginPage = (props) => {
  const {
    ...
    thirdPartyAuthContext: {
      providers, currentProvider, secondaryProviders, finishAuthUrl, platformName, errorMessage: thirdPartyErrorMessage,
    },
  ...
  useEffect(() => { sendPageEvent('login_and_registration', 'login'); }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    if (loginErrorCode) {
      setErrorCode(prev => ({ type: loginErrorCode, count: prev.count + 1, context: { ...loginErrorContext } }));
    }
  }, [loginErrorCode, loginErrorContext]);

  useEffect(() => {
    if (thirdPartyErrorMessage) {
      setErrorCode(prev => ({ type: TPA_AUTHENTICATION_FAILURE, count: prev.count + 1, context: { errorMessage: thirdPartyErrorMessage } }));
    }
  ...
  const handleSubmit = (event) => {
    ...
    if (showResetPasswordSuccessBanner) props.dismissPasswordResetBanner();
  ...
  const handleOnChange = (e) => setFormFields(prev => ({ ...prev, [e.target.name]: e.target.value }));
  const handleOnFocus  = (e) => setErrors(prev => ({ ...prev, [e.target.name]: '' }));
  const trackForgotPasswordLinkClick = () => sendTrackEvent('edx.bi.password-reset_form.toggled', { category: 'user-engagement' });

  const { provider, skipHintedLogin } = getTpaProvider(tpaHint, providers, secondaryProviders);

  if (tpaHint) {
    if (thirdPartyAuthApiStatus === PENDING_STATE) return <Skeleton height={36} />;
    if (skipHintedLogin) { window.location.href = getConfig().LMS_BASE_URL + provider.loginUrl; return null; }
    if (provider) return <EnterpriseSSO provider={provider} />;
  }
    ...
    <>
      ...
      <div className="c-logistration">
        ...
        <aside className="c-logistration__left">
          ...
          <Link to="/authn/register" className="btn btn-outline-light btn-pill">Create account</Link>
        </aside>
...
LoginPage.propTypes = {
  ...
  loginResult: PropTypes.shape({ redirectUrl: PropTypes.string, success: PropTypes.bool }),
...
LoginPage.defaultProps = {
  ...
  thirdPartyAuthContext: { currentProvider: null, errorMessage: null, finishAuthUrl: null, providers: [], secondaryProviders: [] },
};

export default connect(
  mapStateToProps,
  { backupFormState: backupLoginFormBegin, dismissPasswordResetBanner, loginRequest, getTPADataFromBackend: getThirdPartyAuthContext },
)(LoginPage);
