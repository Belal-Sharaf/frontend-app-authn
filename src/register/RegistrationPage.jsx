...
const RegistrationPage = (props) => {
  ...
  useEffect(() => {
    if (!userPipelineDataLoaded && thirdPartyAuthApiStatus === COMPLETE_STATE) {
      if (thirdPartyAuthErrorMessage) {
        setErrorCode(prev => ({ type: TPA_AUTHENTICATION_FAILURE, count: prev.count + 1 }));
      }
      if (pipelineUserDetails && Object.keys(pipelineUserDetails).length !== 0) {
        const { name = '', username = '', email = '' } = pipelineUserDetails;
        setFormFields(prev => ({ ...prev, name, username, email }));
  ...
  useEffect(() => {
    if (backendValidations) {
      if (registrationEmbedded) {
        setTemporaryErrors(prev => ({ ...prev, ...backendValidations }));
      } else {
        setErrors(prev => ({ ...prev, ...backendValidations }));
      }
    }
  }, [backendValidations, registrationEmbedded]);

  useEffect(() => {
    if (registrationErrorCode) setErrorCode(prev => ({ type: registrationErrorCode, count: prev.count + 1 }));
  ...
  const handleOnChange = (e) => {
    ...
    if (registrationError[name]) dispatch(clearRegistrationBackendError(name));
  ...
  const handleErrorChange = (fieldName, error) => {
    if (registrationEmbedded) {
      ...
      if (error === '' && errors[fieldName] !== '') setErrors(prev => ({ ...prev, [fieldName]: error }));
    } else {
  ...
  const registerUser = () => {
    ...
    if (currentProvider) { delete payload.password; payload.social_auth_provider = currentProvider; }
    if (flags.autoGeneratedUsernameEnabled) { delete payload.username; }
    ...
    if (!isValid) { setErrorCode(prev => ({ type: FORM_SUBMISSION_ERROR, count: prev.count + 1 })); return; }

    payload = prepareRegistrationPayload(
      payload, configurableFormFields, flags.showMarketingEmailOptInCheckbox, totalRegistrationTime, queryParams,
    );
    dispatch(registerNewUser(payload));
  };

  const handleSubmit = (e) => { e.preventDefault(); registerUser(); };

  useEffect(() => { if (autoSubmitRegForm && userPipelineDataLoaded) registerUser(); },
      ...
      <>
        ...
        {autoSubmitRegForm && !errorCode.type ? (
          ...
          <div className="c-logistration">
            {/* LEFT — hero + CTA to login */}
            <aside className="c-logistration__left">
              {getConfig().LOGO_URL && (
                <div className="c-logistration__brand">
                  <img src={getConfig().LOGO_URL} alt={platformName} />
                </div>
              )}
              <div className="hero">
                <h1>
                  Start learning <br />
                  <span className="hl">with {platformName}</span>
                </h1>
  ...
  if (tpaHint) {
    ...
    if (skipHintedLogin) { window.location.href = getConfig().LMS_BASE_URL + provider.registerUrl; return null; }
...
RegistrationPage.defaultProps = { handleInstitutionLogin: null, institutionLogin: false };
